...
- 🤖 **Research Ready**: Compare baseline vs deep learning performance
- 📈 **ROUGE/BLEU Evaluation**: Built-in metrics for model comparison
...
2. Install dependencies:
   ```bash
   # Basic dependencies (rule-based system)
...
3. Run the application:
   ...
   python cricsense_match_summary.py --train-dl
   
   # Use deep learning model
   python cricsense_match_summary.py --use-dl --match-id 1343941
   ```

## 🗂️ Data Setup

This app expects SA20 match CSVs in a local folder named `sa20 data/` inside the project.

- Place your match CSV files in: `sa20 data/`
...
```bash
python cricsense_match_summary.py --non-interactive --data-dir "sa20 data"
```
...
- `--data-dir`: Directory containing match CSV files (default: `sa20 data`)
...
- `--train-dl`: Train deep learning model on your data
- `--use-dl`: Use deep learning model for summaries
- `--dl-model-path PATH`: Path to trained model (default: `cricket_summary_model`)
- `--max-examples N`: Maximum examples for training (default: 15)

### Research Comparison

```bash
# Generate baseline summary
python cricsense_match_summary.py --non-interactive --match-id 1343941

# Generate deep learning summary
python cricsense_match_summary.py --non-interactive --match-id 1343941 --use-dl

# Train and compare models
python cricsense_match_summary.py --train-dl --max-examples 20
```
...
This project provides both approaches for research:

1. **Rule-Based Baseline**: Template-driven summaries (your original system)
2. **Deep Learning Model**: GPT-2 transformer trained on cricket data

### Research Workflow

```bash
# Step 1: Generate baseline summaries
python cricsense_match_summary.py --non-interactive --export-json baseline.json

# Step 2: Train deep learning model
python cricsense_match_summary.py --train-dl --max-examples 20

# Step 3: Generate deep learning summaries
python cricsense_match_summary.py --non-interactive --export-json dl.json --use-dl

# Step 4: Compare results using ROUGE/BLEU metrics
```

### Academic Value

- **Novel Application**: First cricket summary deep learning model
- **Real Data**: SA20 ball-by-ball data
- **Reproducible**: Complete methodology
- **Publishable**: Strong research contribution
